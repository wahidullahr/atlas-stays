...
function SubmitBtn() {
  const t = useTranslations('RentPage.listForm');
...
export const RentListForm = () => {
  const t = useTranslations('RentPage.listForm');
  const locale = useLocale() as 'en' | 'fr' | 'ar';
  const [state, formAction] = useActionState(submitContact, initialState);

  return (
    <section id="list-form" className="bg-surface py-16 framer:py-24 overflow-hidden">
        ...
        {state.success ? (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 framer:gap-5 mb-4 framer:mb-5">
              <div>
                <label htmlFor="rent-name" className="block text-[13px] font-medium text-foreground mb-1.5">
                  {t('name')}
                </label>
                <input type="text" id="rent-name" name="name" required className={inputClass} />
                {state.errors?.name && <p className="text-red-500 text-[12px] mt-1">{state.errors.name[0]}</p>}
              </div>
              <div>
                <label htmlFor="rent-email" className="block text-[13px] font-medium text-foreground mb-1.5">
                  {t('email')}
                </label>
                <input type="email" id="rent-email" name="email" required className={inputClass} />
                {state.errors?.email && <p className="text-red-500 text-[12px] mt-1">{state.errors.email[0]}</p>}
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 framer:gap-5 mb-4 framer:mb-5">
              <div>
                <label htmlFor="rent-phone" className="block text-[13px] font-medium text-foreground mb-1.5">
                  {t('phone')}
                </label>
                <input type="tel" id="rent-phone" name="phone" required className={inputClass} />
                {state.errors?.phone && <p className="text-red-500 text-[12px] mt-1">{state.errors.phone[0]}</p>}
              </div>
              <div>
                <label htmlFor="rent-city" className="block text-[13px] font-medium text-foreground mb-1.5">
                  {t('city')}
                </label>
                <select id="rent-city" name="city" required className={inputClass}>
            ...
            <div className="mb-4 framer:mb-5">
              <label htmlFor="rent-units" className="block text-[13px] font-medium text-foreground mb-1.5">
                {t('units')}
              </label>
              <input type="number" id="rent-units" name="units" min={1} required className={inputClass} />
            </div>
            <div className="mb-5 framer:mb-6">
              <label htmlFor="rent-message" className="block text-[13px] font-medium text-foreground mb-1.5">
                {t('message')}
              </label>
              <textarea id="rent-message" name="message" rows={3} required className={`${inputClass} resize-none`} placeholder={t('message_placeholder')} />
